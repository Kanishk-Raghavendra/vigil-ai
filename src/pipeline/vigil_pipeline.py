...
class VIGILPipeline:
    ...
    def run(self, image_path: str, cache: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            claims = self._extract_atomic_claims(caption)
    ...
    @staticmethod
    def _extract_atomic_claims(caption: str) -> List[Any]:
        """Extract verifiable atomic claims from a generated caption.

        Current implementation is rule-based and intentionally lightweight:
        1. Parse candidate claims from surface text patterns.
        2. Filter claims for clarity and verifiability.

        Why this stage matters:
        - Verification is more reliable on short atomic statements than on full
          free-form captions.
        - It separates generation quality from grounding quality.

        Upgrade path (recommended for publication-grade robustness):
        - Introduce a spaCy dependency parser to split coordinated clauses,
          resolve subjects/objects, and normalize relation predicates.
        - Keep this helper as a stable interface so extractor internals can be
          swapped without changing downstream verification/evaluation code.
        """
        # Rule-based extraction provides a deterministic and fast baseline.
        claims = ClaimExtractor.extract_claims(caption)

        # Filtering removes noisy/ambiguous candidates and keeps only claims
        # that are suitable for image-grounded verification.
    ...
    def _generate_explanation(score: float, threshold: float, decision: str) -> str:
        ...
        if decision == "TRUST":
            if score > 0.7:
                return (
                    f"High confidence alignment (score {score_pct:.1f}% > threshold {threshold_pct:.0f}%)"
                )
            return (
                f"Adequate confidence alignment (score {score_pct:.1f}% >= threshold {threshold_pct:.0f}%)"
            )

        if score < 0.2:
            return (
                f"Low confidence alignment (score {score_pct:.1f}% << threshold {threshold_pct:.0f}%)"
            )
